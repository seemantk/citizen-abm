 ...
 */
d3.select("#pause")
  .on("click", function() { anim.pause = true; })
;
d3.select("#play")
  .on("click", function() {
      anim.fwd = true;
...
var uri = {
      ...
      ,  listing: "results/json_viz/noNoise.list"
    }
;
...
queue()
    ...
    .defer(d3.text, uri.base + uri.listing) // comma separated list of files
    .await(function(error, font, listing) {
        listing = listing.split(",");

        world.font(font);
        /*
         * Construct a dropdown to hold the names of the available
